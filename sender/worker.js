...
class Worker extends EventEmitter {
    constructor(client, testFs) {
        ...
        this.on(INIT, (data) => {
            ...
            log.info('asking server to get snapshot list for %s', this.fs.name);
        ...
        this.on(SNAPSHOT_LIST, (remote) => {
            ...
            getSnapshotList(this.fs, (fs) => {
                if (fs.list.length === 0) {
                    ...
                    log.info('found the following snapshots on my filesystem %s', this.fs.name);
                    ...
                    if (commonList.length === 0) {
                        ...
                        log.info('an initial seed needed');
                        log.info('initial snapshot: %s', fs.list[0]);
                        let recvFs = {
                            name: this.fs.name,
                            incremental: false,
                        ...
                        log.info('asking the server to start up a recv for %s', recvFs.name);
                        ...
                        log.info('the server has all my snapshots');
                        log.info('my snapshots: %s', fs.list.join(', '));
                        log.info('server snapshots: %s', remote.list.join(', '));

                        log.info('asking server to end connection');
                        this.client.write(message(END, null));
                    } else {
                        // incremental snapshot needed, find the oldest common snapshot
                        log.info('an incremental send is needed');
                        log.info('from: %s to: %s', commonList[commonList.length - 1], fs.list[commonList.length]);
                        let recvFs = {
                            name: this.fs.name,
                            incremental: true,
